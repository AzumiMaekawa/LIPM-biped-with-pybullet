"""
bipedal walking pattern generation with inverted pendulum model
reference: https://github.com/AtsushiSakai/PythonRobotics/blob/master/Bipedal/bipedal_planner/bipedal_planner.py
"""
import numpy as np
import math
...
class BipedalPlanner(object):
    ...
    def inverted_pendulum_dynamics(self, x, x_dot, px_star, y, y_dot, py_star, z_c, time_width):
        time_split = 100
    ...
    def walk(self, T_sup=0.8, z_c=0.8, a=10, b=1, plot=False):
        ...
        for i in range(len(self.ref_footsteps)):
            xi, xi_dot, yi, yi_dot = self.inverted_pendulum_dynamics(
                xi, xi_dot, px_star, yi, yi_dot, py_star, z_c, T_sup)

            time += T_sup
            n += 1

            # caluculate px, py, x_, y_, vx_, vy_
            f_x, f_y, f_theta = self.ref_footsteps[n - 1]
            rotate_mat = np.array(
                [[math.cos(f_theta), -math.sin(f_theta)],
            ...
            x_, y_ = list(
                np.dot(rotate_mat_next,
                       np.array([f_x_next / 2., math.pow(-1, n)*f_y_next / 2.])))
            vx_, vy_ = list(
                np.dot(
                    rotate_mat_next,
                    np.array(
            ...
            if plot:
                ...
                for c in range(len(self.com_trajectory)):
                    if c > len(com_trajectory_for_plot):
                        ...
                        plt.cla()  # clear axes
                        ax.set_zlim(0, z_c * 2)
                        ax.set_aspect('equal', 'datalim')  # axes setting
                        ...
                        for j in range(len(self.ref_p)):
                            ...
                            r = math.sqrt(
                                math.pow(foot_width / 3., 2) +
                                math.pow(foot_height / 2., 2))
                            rec = pat.Rectangle(
                                xy=(self.ref_p[j][0] + r * math.cos(angle),
                                    self.ref_p[j][1] + r * math.sin(angle)),
                                width=foot_width,
                                height=foot_height,
                                angle=self.ref_p[j][2] * 180 / math.pi,
                                color="blue", fill=False, ls=":")
                            ax.add_patch(rec)
                        ...
                        for j in range(len(self.act_p)):
                            ...
                            r = math.sqrt(
                                ...
                                math.pow(foot_height / 2., 2))
                            rec = pat.Rectangle(
                                xy=(self.act_p[j][0] + r * math.cos(angle),
                                    self.act_p[j][1] + r * math.sin(angle)),
                                width=foot_width,
                                height=foot_height,
                                angle=self.act_p[j][2] * 180 / math.pi,
